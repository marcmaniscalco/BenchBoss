...
If a submission fails validation (missing title, unparseable date/time, or a non-positive duration), the bot replies with an ephemeral error message and a **Fix and Retry** button instead of just closing the modal. Discord does not allow responding to a modal submission with another modal — only a button/select click can open one — so the failed submission's field values are stashed server-side (a short-lived, ~10 minute DynamoDB item) and the button click reopens the modal prefilled with everything you typed. The field that failed gets a 🔴* label naming the problem — e.g. `🔴* Date/Time — YYYY-MM-DD H:MM AM/PM` — since Discord text inputs only show their placeholder text when empty, and a reopened field always has your previous (invalid) value in it, so the label is the only place a fix-it hint is guaranteed to actually be seen. If you wait too long before clicking Retry, the button replies that the retry has expired and you'll need to run the command again.
